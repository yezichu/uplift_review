...
from loss.causal_loss import CFRNetlossWrapper
...
class CFRNet(nn.Module):
    ...
    def compute_loss(self, batch):
        ...
        phi_t = phi[mask_1]
        phi_c = phi[mask_0]
        return self.loss_fn(y_pred, y, phi_t, phi_c, w, self.alpha, self.distance)
...
class CFRNetEstimator:    
    def __init__(self, x_dim, hidden_dim, lr = 1e-3):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.model = CFRNet(x_dim = x_dim, hidden_dim = hidden_dim).to(self.device)
